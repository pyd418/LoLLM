...
from src.bert_feature_extractor import BertLayer
from src.decoder import DistMult, ConvE, ConvTransE, ConvKB
import src.layers as layers


class LinkPredictor_SA(nn.Module):

    def __init__(self, num_nodes, num_rels, args, use_cuda=False):

        super(LinkPredictor_SA, self).__init__()
    ...
    def regularization_loss(self, embedding):
        # dec_weight = self.decoder.modules.w_relation.weight.pow(2)
    ...
    def get_score(self, e1, rel, target, embedding):
        ...
        decoder_score = self.calc_score(e1, rel, target)    # decoder算出loss值
        # predict_loss = F.binary_cross_entropy(decoder_score, target)
        predict_loss = decoder_score

        if self.reg_param != 0.0:   # 是否进行归一化
