...
def generate_sampled_graph_and_labels(triplets, sample_size,
    ...
    # build DGL graph
    logging.info("# sampled nodes: {}".format(len(uniq_v)))
    logging.info("# sampled edges: {}".format(len(src)))
...
def build_graph_from_triplets(num_nodes, num_rels, triplets):
    ...
    logging.info("# nodes: {}, # edges: {}".format(num_nodes, len(src)))
    return g, rel, norm


def build_test_graph(num_nodes, num_rels, edges):
    src, rel, dst = edges.transpose()
    logging.info("Test graph:")
...
def negative_sampling(pos_samples, num_entity, negative_rate):
    ...
    neg_samples = np.tile(pos_samples, (negative_rate, 1))  # 复制pos维度
    labels = np.zeros(size_of_batch * (negative_rate + 1), dtype=np.float32)    # 所有正，负例labels
    labels[: size_of_batch] = 1
    # TODO: pick negative samples only with same relations
    values = np.random.randint(num_entity, size=num_to_generate)    # num_to_generate个随机整数，随即替换一个实体
    #values = np.random.choice(tot_entities, size=num_to_generate, replace=False)
    
    choices = np.random.uniform(size=num_to_generate)   #一半替换头，剩下替换尾
    subj = choices > 0.5
    obj = choices <= 0.5
...
def sim_sim_connect(pos_samples, all_triplets, num_rels):
    ...
    if len(addl_samples) == 0:
        ...
        logging.info("Adding %d sim-sim edges" % (unique_samples.shape[0] - pos_samples.shape[0]))
