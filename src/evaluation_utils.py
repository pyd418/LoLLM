...
def ranking_and_hits(test_graph, model, test_triplets, e1_to_multi_e2, network, fusion="graph-only",
                     sim_relations=False, write_results=False, debug=False, epoch=None, max_mrr=None):
    # print(model)
    ...
    for i in range(0, end, batch_size):
        ...
        # logging.info('head @{0}'.format(e1))
        # logging.info('relation @{0}'.format(rel))
        # logging.info('tail @{0}'.format(e2))
        # # print(rel)
        # logging.info('top tail @{0}'.format(argsort1[0:, 0:10]))
    ...
    if max_mrr != None:
        if np.mean(1. / np.array(ranks_left)) > max_mrr:
            max_mrr = np.mean(1. / np.array(ranks_left))
    ...
    logging.info('Max reciprocal rank: {0}'.format(max_mrr))
    ...
    return np.mean(1. / np.array(ranks)), max_mrr
